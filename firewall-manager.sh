...
apply_rules(){
  ...
  # Remove any previous version of our table (ensures sets are recreated cleanly)
  ...
  local tmp_rules; tmp_rules=$(mktemp)
  {
    ...
    if ((${#BLOCKED_CLEAN[@]})); then
      ...
      for ip in "${BLOCKED_CLEAN[@]}"; do
        [[ -z "$ip" ]] && continue
        if (( first )); then printf "%s" "$ip"; first=0; else printf ", %s" "$ip"; fi
      done
      echo " }"
    fi
    echo "  }"
...
EOF
    ...
    ((${#BLOCKED_CLEAN[@]})) && echo "    ip daddr @blocked_ips drop"
    echo "  }"
...
add_ports_interactive(){ 
  ...
  local changed
  changed=$(parse_and_process_ports "add" "$proto_file" "$input_ports" 2>/dev/null)
...
remove_ports_interactive(){
  ...
  [[ -z "$input_ports" ]] && return 0
  local changed
  changed=$(parse_and_process_ports "remove" "$proto_file" "$input_ports" 2>/dev/null)
  if [[ "$changed" =~ ^[0-9]+$ ]] && (( changed > 0 )); then
    echo -e "${YELLOW}Applying firewall...${NC}"
    apply_rules --no-pause
  else
  ...
  while true; do
    ...
    case $choice in
      1) read -r -p "Enter IPs/CIDRs to ADD: " ips < /dev/tty
         if [[ -n "$ips" ]]; then local changed; changed=$(parse_and_process_ips "add" "$ips"); [[ "$changed" =~ ^[0-9]+$ ]] && (( changed > 0 )) && did_change=1; fi ;;
      2) read -r -p "Enter IPs/CIDRs to REMOVE: " ips < /dev/tty
         if [[ -n "$ips" ]]; then local changed; changed=$(parse_and_process_ips "remove" "$ips"); [[ "$changed" =~ ^[0-9]+$ ]] && (( changed > 0 )) && did_change=1; fi ;;
