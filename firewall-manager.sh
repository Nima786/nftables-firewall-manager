...
# =================================================================
#  Interactive NFTABLES Firewall Manager - v7.8 (Set-Optimized)
# =================================================================
...
# - CHANGE: Replaced inefficient per-IP block rules with a highly
...
parse_and_process_ports(){
  ...
  for item in "${port_items[@]}"; do
    ...
    if [[ "$item" == *-* ]]; then
 ...
 elif [[ "$action" == "remove" ]] && grep -q "^${port}$" "$proto_file"; then sed -i "/^${port}$/d" "$proto_file"; ((count++));
 fi
        done
      ...
      if [[ "$action" == "add" ]] && ! grep -q "^${item}$" "$proto_file"; then echo "$item" >> "$proto_file"; ((count++));
 echo -e " -> ${GREEN}Port $item added.${NC}" >&2
      ...
      else echo -e " -> ${YELLOW}Port $item not found.${NC}" >&2;
 fi
    elif [[ -n "$item" ]]; then echo -e " -> ${RED}Invalid input: $item${NC}" >&2;
 fi
  done
  printf '%s\n' "$count"
}
...
valid_ipv4_cidr(){
  ...
  if [[ -n "$mask" ]]; then [[ "$mask" =~ ^[0-9]+$ ]] && ((mask>=0 && mask<=32)) || return 1;
 fi
  return 0
}
parse_and_process_ips(){
  ...
  for raw in "${items[@]}"; do
    ...
    [[ -z "$item" ]] && continue
    if ! valid_ipv4_cidr "$item"; then echo -e " -> ${RED}Invalid IPv4/CIDR: $item${NC}" >&2; continue;
 fi
    if [[ "$action" == "add" ]]; then
      if ! grep -qxF "$item" "$BLOCKED_IPS_FILE"; then echo "$item" >> "$BLOCKED_IPS_FILE"; canonicalize_blocklist_file; ((count++));
 echo -e " -> ${GREEN}$item added.${NC}" >&2
      else echo -e " -> ${YELLOW}$item already present.${NC}" >&2;
 fi
    else
      ...
      else echo -e " -> ${YELLOW}$item not found.${NC}" >&2;
 fi
    fi
  done
  printf '%s\n' "$count"
...
manage_ips_menu(){
 ...
 3) if command -v less >/dev/null 2>&1; then less -S "$BLOCKED_IPS_FILE" </dev/tty || true
         ...
         else cat "$BLOCKED_IPS_FILE"; press_enter_to_continue;
 fi
         continue ;;
      4) break ;;
