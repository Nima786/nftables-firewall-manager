...
# =================================================================
#  Interactive NFTABLES Firewall Manager - v8.1 (set-based, safe)
# =================================================================
# - No global flush on Apply (preserves other tables like Docker/NAT)
# - Blocklist in one named set (fast); overlaps filtered
# - Docker-aware FORWARD; ICMP allow for PMTU/diagnostics
# - Toggle blocklist enforcement ON/OFF
# - Flush All really wipes rules + persistence + config dir
# =================================================================
...
CHUNK_SIZE=300               # elements per add-element chunk
...
# ---- Filter out overlapping CIDRs: keep broader, skip enclosed ----
filter_blocklist_nonoverlap() {
  awk '
  function pow2(x) { return 2^x }
  function ip2int(a,b,c,d) { return (((a*256)+b)*256+c)*256+d }
  function parse(line,   ip,mask,a,b,c,d,n,size,start,end) {
...
# ---------- table/set create (no global flush) ----------
create_table_and_chains() {
  # Drop only our table if it exists
  ...
  # Build table with empty set and chains
  local tmp; tmp=$(mktemp)
  {
...
add_elements_to_set() {
  # Load, sanitize, filter overlaps, then add in chunks
  ...
  while (( i < total )); do
    ...
    if ! nft -f "$tmp" 2>/dev/null; then
      # fallback: per-element (ignore conflicts)
      for ((j=i; j<end; j++)); do
...
apply_rules() {
  ...
  # (Re)create our table and chains (no global flush)
  create_table_and_chains

  # Always populate the set from file (even if enforcement OFF),
  # so you can toggle ON instantly without reloading elements.
  add_elements_to_set
