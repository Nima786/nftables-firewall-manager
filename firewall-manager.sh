...
SET_NAME="blocked_ips"          # ipv4 set name (matches your logs)
...
# ---------- table/create (no embedded elements) ----------
...
EOF
  ...
  # Try to load; if "file exists" sneaks in, wipe just our table and retry once.
...
add_elements_to_set(){
  ...
  while (( i < total )); do
    ...
    if ! nft -f "$tmp" 2>/dev/null; then
      # fallback per-element if a batch fails for any reason
      for ((j=i; j<end; j++)); do
...
apply_rules(){
  ...
  create_table_and_chains     # no global flush
  add_elements_to_set         # elements added AFTER table creation
