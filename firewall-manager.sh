...
# =================================================================
#  Interactive NFTABLES Firewall Manager - v5.9
# =================================================================
# - Detects/whitelists SSH port automatically
# - Installs dependencies (nftables, curl)
# - Optional full system upgrade via --upgrade flag
# - Robust to empty files with set -euo pipefail
# - Proper blocklist via named set (ip saddr on input, ip daddr on output/forward)
# - No silent "hangs": apt output is shown
...
apply_rules() {
  ...
  # Build blocklist elements (safe when empty)
  local blocked_elems=""
...
main_menu() {
  while true; do
    clear
    echo "==============================="
    echo " NFTABLES FIREWALL MANAGER v5.9"
    echo "==============================="
