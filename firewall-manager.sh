...
function check_dependencies() {
    ...
    if ! command -v netfilter-persistent &> /dev/null; then
        ...
        if ! command -v netfilter-persistent &> /dev/null; then echo -e "${RED}FATAL: Failed to install 'iptables-persistent'. Please install it manually.${NC}"; exit 1; fi
    ...
    if ! command -v curl &> /dev/null; then
        ...
        if ! command -v curl &> /dev/null; then echo -e "${RED}FATAL: Failed to install 'curl'. Please install it manually.${NC}"; exit 1; fi
