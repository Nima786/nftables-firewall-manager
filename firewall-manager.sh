...
# =================================================================
#  Interactive NFTABLES Firewall Manager - v6.0
# =================================================================
# - Detects & auto-allows the active SSH port
# - Installs dependencies (nftables, curl)
# - Optional apt upgrade via --upgrade flag (no silent "hangs")
# - Robust to empty files (with set -euo pipefail)
# - **Overlap-safe blocklist:** emits per-rule drops (no interval set)
# - Cleans comments/blank lines and de-duplicates blocklist entries
...
# --- SYSTEM UPDATE & DEPS ---
check_dependencies() {
  echo "[+] Checking/Updating apt metadata & dependencies..."
  export DEBIAN_FRONTEND=noninteractive
  apt-get update -y

  if [[ "$DO_UPGRADE" == "true" ]]; then
    echo "[+] Performing apt upgrade (this may take a while)..."
...
detect_ssh_port() {
  local port=""
  # Try to find a listening port owned by sshd
  port=$(
...
# Return a cleaned, de-duplicated list of IPv4/CIDR entries (no comments/blank lines)
get_clean_blocklist() {
  awk '
    /^[[:space:]]*#/ { next }
    /^[[:space:]]*$/ { next }
...
initial_setup() {
  if [ ! -d "$CONFIG_DIR" ]; then
    ...
    # Keep SSH allowed in case sshd changed
    ensure_ssh_in_config
  fi
}
...
apply_rules() {
  ...
  # Robust to empty files; don't let pipefail kill us
  ...
  # Clean, unique blocklist (still may contain overlaps; that's OK now)
  mapfile -t BLOCKED_CLEAN < <(get_clean_blocklist)

  # Build nftables ruleset (emit per-line drop rules instead of an interval set)
...
main_menu() {
  while true; do
    clear
    echo "==============================="
    echo " NFTABLES FIREWALL MANAGER v6.0"
    echo "==============================="
