...
# =================================================================
#  Interactive NFTABLES Firewall Manager - v8.0 (Final)
# =================================================================
...
# - Auto-populate & canonicalize blocklist from source
...
#   - Fixed bug in port management function that caused syntax
#     errors by ensuring all status messages go to stderr.
# =================================================================
...
parse_and_process_ports(){
  ...
  for item in "${port_items[@]}"; do
    ...
    if [[ "$item" == *-* ]]; then
      ...
      if [[ "$action" == "add" ]]; then
        if ! grep -q "^${item}$" "$proto_file"; then
  ...
  # This is the ONLY line that outputs to stdout for variable capture.
  printf '%s\n' "$count"
}
